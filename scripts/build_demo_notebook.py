...
def main() -> None:
    ...
    notebook["cells"] = [
        ...
        nbf.v4.new_markdown_cell(
            ...
            "nine regions use the same label ontology. It uses QC-passing "
            "automatic FetalSynthSeg predictions from cases labeled neurotypical, 22.7–34.8 weeks, with a quadratic centered-age model "
        ...
        nbf.v4.new_code_cell(
            ...
            "    cached_metadata = json.loads(FETA_METADATA_PATH.read_text())\n"
            "    if (cached_metadata.get('segmentation_method') == 'FetalSynthSeg v1 automatic prediction'\n"
            "            and cached_metadata.get('checkpoint_sha256')):\n"
            "        feta_curves = pd.read_csv(FETA_CURVES_PATH)\n"
            "        feta_metadata = cached_metadata\n"
            ...
            "        feta_curves, feta_metadata, control_tissues, control_volumes, control_qc = build_feta_matched_reference(\n"
            "            feta_root, prediction_dir=FETA_REFERENCE_DIR/'fetalsynthseg_predictions',\n"
            "            checkpoint=CHECKPOINT, device_name='auto', degree=2,\n"
            "        )\n"
